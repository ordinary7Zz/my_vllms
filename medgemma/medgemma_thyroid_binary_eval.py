...
def prob_malignant_from_logits(model, processor, img, prompt: str, prefer_leading_space: bool = True) -> Tuple[float, Dict[str, float]]:
    ...
    with open(args.out_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for r in tqdm(labels, desc="Infer"):
            ...
            try:
                ...
                continue
            try:
                p1, dbg = prob_malignant_from_logits(model, processor, img, MEDGEMMA_PROMPT, prefer_leading_space=True)
            except Exception:
                p1, dbg = prob_malignant_from_logits(model, processor, img, MEDGEMMA_PROMPT, prefer_leading_space=False)
